...
6. 반드시 100% 한글로만 작성하세요. 영어 단어, 알파벳, 한자(漢字/中文) 등 한글이 아닌 문자를 단 한 글자도 섞지 마세요.
...
언어 규칙: name과 description 모두 반드시 100% 한글로만 작성하세요.
영어 단어, 알파벳, 한자(漢字/中文) 등 한글이 아닌 문자를 단 한 글자도 섞지 마세요.
...
{{"gold": 0, "hp": 0, "item_name": null, "item_kind": null}}"""
    ...
    async def generate_special_event(self, player_state: PlayerState) -> Optional[str]:
        ...
        prompt = f"""당신은 텍스트 RPG의 퀘스트 생성기입니다. 이 모험가의 상태를 바탕으로 하나의 간단한 퀘스트 훅을 생성하세요 (1-2 문장).
...
규칙:
1. 위 이야기 요약의 배경, 인물, 사건과 자연스럽게 이어지는 퀘스트만 생성하세요. 이야기와 무관한 엉뚱한 설정(새로운 마을, 관계없는 상인 등)을 만들지 마세요
2. 플레이어의 현재 레벨과 위치에 맞게 하세요
3. 흥미롭지만 간결하게 하세요

언어 규칙: 반드시 100% 한글로만 작성하세요. 영어 단어, 알파벳, 한자(漢字/中文) 등 한글이 아닌 문자를 단 한 글자도 섞지 마세요."""
