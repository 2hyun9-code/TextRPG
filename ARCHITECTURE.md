...
- **스킬**: 직업별 고유 스킬 1개, 정신력(MP) 소모 (skills_db.py)
  - 전사 강타(2배) / 도적 급습(1.8배) / 마법사 화염구(지능×2 추가) /
    성기사 심판의 빛(1.5배+자기회복) / 레인저 관통 사격(1.7배, 방어 무시)
  - MP는 직업별 기본치(마법사 80 ~ 전사 30) + 레벨업 성장, 여관에서 회복
- **상태 이상**: 일반 몬스터 반격 시 12% 중독(3턴, 매턴 레벨×2 피해),
  보스 반격 시 20% 기절(1턴 행동 불가). 전투 종료/여관에서 해제
...
행동 후 AI가 자기 서사를 다시 읽고 실제 변화를 추출한다 (`extract_story_events`):

1. 서사에 명확히 서술된 획득/손실만 JSON으로 추출 (골드/체력/아이템)
2. **코드가 범위를 강제**: 골드 상한 = 레벨 비례, 서사 피해로는 사망 불가(최소 1),
   아이템 스탯은 코드가 결정 (전투 전리품보다 약간 약함)
3. 추출 실패 시 서사만 출력 (게임 상태 변화 없음 - 안전 폴백)
4. 적용된 변화는 채팅 이벤트 로그 + AI 기억에 기록
...
- **저장 슬롯 3개**: `save_slot_N.json` — 설정 모달에서 저장/불러오기
- **메모리 캐시**: 매 요청마다 파일을 읽지 않음
- 구버전 세이브 호환: 새 필드는 기본값으로 자동 로드, 마나는 직업/레벨 기준 소급 적용
...
| POST | `/api/combat/skill` | 스킬 사용 (MP 소모) |
...
| GET | `/api/saves` | 저장 슬롯 목록 (3개) |
| POST | `/api/saves/save` | 슬롯에 저장 |
| POST | `/api/saves/load` | 슬롯 불러오기 |
| GET | `/api/ollama/models` | 설치된 모델 목록 + 현재 모델 |
| POST | `/api/ollama/model` | 모델 변경 (재시작 시 초기화) |
